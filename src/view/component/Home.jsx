    ...
    <div key={index} className={isFirst ? "story is-first" : "story"}>
        <div className="story__thumb">
            <a href={value.link} target="_blank" title={value.title}>
                <img src={converImg(value['content:encoded'] ? value['content:encoded'] : value.content)} alt={value.title} title={value.title} />
            </a>
        </div>

        <h4 className="story__heading">
            <a href={value.link} title={value.title} target="_blank">{value.title}</a>
        </h4>
        ...
        <img className="lazy" data-src={converImg(value.content)} src={converImg(value.content)} />
    </div>
...
export default function Home(props) {
    ...
    const listMenu = urlTitle.map((value, index) =>
        <a key={index} className={indexSelected === index ? "popular is-active" : "latest"}
            href="#" onClick={() => handleClick(props, index)} >
            <span>{value}</span>
    ...
    return (<div className="wrapper homepage_page">
